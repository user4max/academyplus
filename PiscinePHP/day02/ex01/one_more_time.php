	...
	{	
		...
		{	
			if (preg_match("/^[0-9]*$/", $time[0]) && preg_match("/^[0-9]*$/", $time[1])
				...
				&& preg_match("/^[0-9]*$/", $date[1]) && strlen($time[0]) == 2 && strlen($time[1]) == 2 
				&& strlen($time[2]) == 2 && intval($time[0]) < 24 && intval($time[1]) < 60 && intval($time[2]) < 60 
				&& strlen($date[3]) == 4 && (strlen($date[1]) == 1 || strlen($date[1]) == 2) && intval($date[1]) < 32 
				&& preg_match("/^[a-zA-Z]*$/", $date[2]) && preg_match("/^[a-zA-Z]*$/", $date[0]))
				{
